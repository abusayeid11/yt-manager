...
public class TranscriptService {
    ...
    public String executeScraper(String url) throws Exception {
        ...
        if (pythonExec == null || pythonExec.isBlank()) {
            pythonExec = System.getProperty("os.name").toLowerCase().contains("windows") ? "python" : "python3";
        }
        ...
        ProcessBuilder pb = new ProcessBuilder(pythonPath.toString(), scriptPath.toString(), url);
    ...
    public List<Map<String, Object>> parseTranscriptJson(String jsonResponse) {
    ...
    public List<TranscriptSegment> createSentenceAwareSegments(List<Map<String, Object>> snippets, int segmentDurationSeconds) {
        ...
        for (Map<String, Object> snippet : snippets) {
            ...
            if (start >= windowEnd && (lastEndedWithSentence || start >= windowEnd + segmentDurationSeconds * 0.5)) {
                TranscriptSegment segment = new TranscriptSegment(
                    formatTime(currentWindowStart), currentWindowStart, windowEnd, currentText.toString().trim());
        ...
        if (currentText.length() > 0) {
            segments.add(new TranscriptSegment(formatTime(currentWindowStart), currentWindowStart, windowEnd, currentText.toString().trim()));
        }
        return segments;
    }
    ...
    private boolean isSentenceEnd(String text) {
        ...
        return SENTENCE_END.matcher(trimmed).find() || trimmed.endsWith(".") || trimmed.endsWith("!") || trimmed.endsWith("?");
    }
